...
/// }    
    ...
    public class EthGetFilterChangesForEthNewFilter : RpcRequestResponseHandler<EthNewFilterLog[]>
        {
            public EthGetFilterChangesForEthNewFilter(RpcClient client) : base(client, ApiMethods.eth_getFilterChanges.ToString()) { }

            public async Task<EthNewFilterLog[]> SendRequestAsync( HexBigInteger filterId, string id = Constants.DEFAULT_REQUEST_ID)
            {
