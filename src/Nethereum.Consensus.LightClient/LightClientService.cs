...
namespace Nethereum.Consensus.LightClient
{
    public class LightClientService
    {
        ...
        public async Task InitializeAsync(CancellationToken cancellationToken = default)
        {
            ...
            // Bootstrap root: use the configured weak-subjectivity checkpoint when
            // supplied; otherwise derive it from the beacon node's latest finality
            // update (the finalized header's root), matching the wallet's
            // VerifiedBalanceService bootstrap. Lets the light client bootstrap
            // against a trusted beacon endpoint without an operator-supplied root.
            ...
            if (IsEmptyRoot(bootstrapRoot))
            {
                ...
                // Pin the derived anchor so ValidateBootstrap checks the bootstrap
                // header against the root we actually fetched it for.
                _config.WeakSubjectivityRoot = bootstrapRoot;
            }
        ...
        /// </summary>
        /// <summary>
        /// Reason the most recent <see cref="TryApplyUpdate"/> rejected an update, or
        /// null if the last one applied. Diagnostic only — lets the hosted poller log
        /// why the head stopped advancing.
        /// </summary>
