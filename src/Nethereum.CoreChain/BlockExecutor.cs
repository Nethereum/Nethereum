    ...
    public sealed class BlockExecutor
    {
            ...
            CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                //     exactly at block 1,920,000). 58 child-DAO
        ...
        /// + the DAO contract itself into the WithdrawDAO contract: for every
        /// address in the drain list, add its balance to the withdraw contract
        /// and zero the source balance.
        /// No EVM rule change — this runs at Homestead EVM rules. The
        /// withdrawal contract did not exist before block 1,920,000; crediting
        /// it creates an empty account on first credit.
        /// </summary>
        private static async Task ApplyDaoForkDrainAsync(IStateStore stateStore, CancellationToken ct)
        {
            ...
            foreach (var addr in Forks.DaoForkConstants.DrainList)
            {
                ...
                if (bal.IsZero)
                {
                    // The canonical rule sets the balance to 0 here — but since
