...
{
    /// <summary>
    /// Mainnet DAO fork (block 1,920,000) constants. Verbatim from geth
    /// <c>params/dao.go DAODrainList()</c> at
    /// https://github.com/ethereum/go-ethereum/blob/master/params/dao.go.
    /// Geth ref: <c>consensus/misc/dao.go ApplyDAOHardFork</c>.
    ///
    /// 115 entries: 57 child-DAO accounts × 2 (each with a paired
    /// "extra-balance" account) + the main DAO + the main DAO's
    /// extra-balance account. Geth's list has the DAO contract
    /// (<c>0xbb9bc244...</c>) at index 113 (second-to-last) and an
    /// extra-balance account (<c>0x807640a1...</c>) as the final entry.
    ///
    /// EVM rules at this block equal Homestead — DAO is consensus-only,
    /// not an EVM change. Mainnet block 1,920,000 state-root canary:
    /// <c>0xc5e389416116e3696cce82ec4533cce33efccb24ce245ae9546a4b8f0d5e9a75</c>.
    /// Block hash: <c>0x4985f5ca3d2afbec36529aa96f74de3cc10a2a4a6c44f2157a57d2c6059a11bb</c>.
    /// </summary>
    public static class DaoForkConstants
    {
        /// <summary>
        /// Recipient of every drained balance. Did NOT exist before block
        /// 1,920,000 — geth's CreateAccount + AddBalance creates the account
        /// on first credit (our CreditAsync does the same).
        /// </summary>
        public const string WithdrawContractAddress = "0xbf4ed7b27f1d666546e30d74d50d173d20bca754";

        /// <summary>
        /// All 115 source accounts, geth's exact order. Includes child-DAO
        /// extra-balance accounts (created when DAO governance split a
        /// child DAO with an "extraBalanceAccount" field for residual ETH).
        /// </summary>
        public static readonly string[] DrainList = new[]
        {
            ...
            "0xbb9bc244d798123fde783fcc1c72d3bb8c189413",  // the DAO contract itself
            "0x807640a13483f8ac783c557fcdf27be11ea4ac7a",  // DAO extra-balance
        };
    }
}
