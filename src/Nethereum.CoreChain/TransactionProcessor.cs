    ...
    public class TransactionProcessor
    {
            ...
            bool traceEnabled = false)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // the persisted state trie diverges from geth's, whose
                    // Finalise drops destructed objects (statedb.go:760).
        ...
        private async Task PersistExecutionStateChangesAsync(ExecutionStateService executionStateService)
        {
            foreach (var accountKvp in executionStateService.AccountsState)
            {
                ...
                // Geth's state.StateDB.Finalise has the same "only write
                // back dirty storage" invariant.
                ...
                // empty account must persist in the state trie. Geth's
                // core/state/statedb.go Finalise(deleteEmptyObjects:false)
                // keeps every dirty account regardless of emptiness. CALL with
