...
namespace Nethereum.EVM.Execution
{
    ...
    {
        public bool EnableEIP7883 { get; set; }

        public EvmPreCompiledContractsExecution()
        {
        }

        public virtual long GetPrecompileGasCost(string address, byte[] data)
        {
            ...
            switch (address.ToHexCompact().ToLowerInvariant())
            {
                ...
                case "b": // BLS12-381 G1ADD (EIP-2537)
                    return GasConstants.BLS12_G1ADD_GAS;
                case "c": // BLS12-381 G1MSM (EIP-2537)
                    return GetBls12G1MsmGas(dataLen);
                case "d": // BLS12-381 G2ADD (EIP-2537)
                    return GasConstants.BLS12_G2ADD_GAS;
                case "e": // BLS12-381 G2MSM (EIP-2537)
                    return GetBls12G2MsmGas(dataLen);
                case "f": // BLS12-381 PAIRING (EIP-2537)
                    return GetBls12PairingGas(dataLen);
                case "10": // BLS12-381 MAP_FP_TO_G1 (EIP-2537)
                    return GasConstants.BLS12_MAP_FP_TO_G1_GAS;
                case "11": // BLS12-381 MAP_FP2_TO_G2 (EIP-2537)
                    return GasConstants.BLS12_MAP_FP2_TO_G2_GAS;
                case "100": // P256VERIFY (EIP-7951)
                    return GasConstants.P256VERIFY_GAS;
            }
            return 0;
        }

        private long GetModExpGas(byte[] data)
        {
            data = data ?? new byte[0];

            int offset = 0;
            var baseLen = ReadBigEndian(data, offset, 32);
            offset += 32;
            var expLen = ReadBigEndian(data, offset, 32);
            offset += 32;
            var modLen = ReadBigEndian(data, offset, 32);
            offset += 32;
            ...
            int multiplier = EnableEIP7883 ? 16 : 8;
            ...
            {
                iterationCount = multiplier * (expLen - 32) + (expHead > 0 ? expBitLen - 1 : 0);
            }
        ...
        private long GetBn128PairingGas(int dataLen)
        {
            int k = dataLen / 192;
            return 34000L * k + 45000;
        }

        private long GetBlake2fGas(byte[] data)
        {
        ...
        {
            int k = dataLen / 384;
            return GasConstants.BLS12_PAIRING_BASE_GAS + (long)k * GasConstants.BLS12_PAIRING_PER_PAIR_GAS;
        }
        ...
        [Obsolete("Use IsPrecompiledAddress instead (fixed spelling)")]
        public virtual bool IsPrecompiledAdress(string address)
        {
            return IsPrecompiledAddress(address);
        }
...
#if !EVM_SYNC
            // Precompiles requiring Signer/BLS/KZG (async only — Zisk uses ZiskPrecompileProvider instead)
            switch (address.ToHexCompact().ToLowerInvariant())
            {
                case "1":
        ...
        private static BigInteger ReadBigEndian(byte[] data, int offset, int length)
        {
        ...
        public byte[] ModExp(byte[] data)
        {
            data = data ?? new byte[0];
            int offset = 0;
            var baseLenBig = ReadBigInteger(data, offset, 32); offset += 32;
            var expLenBig = ReadBigInteger(data, offset, 32); offset += 32;
            var modLenBig = ReadBigInteger(data, offset, 32); offset += 32;
            if (EnableEIP7883 && (baseLenBig > 1024 || expLenBig > 1024 || modLenBig > 1024))
                throw new System.ArgumentException("MODEXP length exceeded 1024 bytes");
            if (modLenBig == 0) return new byte[0];
            if (baseLenBig > int.MaxValue || expLenBig > int.MaxValue || modLenBig > int.MaxValue)
                throw new System.ArgumentException("MODEXP length too large");
            var baseLen = (int)baseLenBig; var expLen = (int)expLenBig; var modLen = (int)modLenBig;
            var baseVal = ReadBigIntegerUnsigned(data, offset, baseLen); offset += baseLen;
            var expVal = ReadBigIntegerUnsigned(data, offset, expLen); offset += expLen;
            ...
            { var trimmed = new byte[resultBytes.Length - 1]; Array.Copy(resultBytes, trimmed, trimmed.Length); resultBytes = trimmed; }
            Array.Reverse(resultBytes);
            if (resultBytes.Length == modLen) return resultBytes;
            if (resultBytes.Length > modLen)
            { var truncated = new byte[modLen]; Array.Copy(resultBytes, resultBytes.Length - modLen, truncated, 0, modLen); return truncated; }
        ...
        public byte[] ModExp(byte[] data)
        {
            ...
            // EIP-7823 (Osaka): Reject operands exceeding 1024 bytes (checked before modLen==0)
            if (EnableEIP7883 && (baseLenBig > 1024 || expLenBig > 1024 || modLenBig > 1024))
            {
        ...
        public byte[] P256Verify(byte[] data)
        {
            ...
            try
            {
                ...
                var curveParams = Org.BouncyCastle.Asn1.Nist.NistNamedCurves.GetByName("P-256");
                ...
                var xBC = new Org.BouncyCastle.Math.BigInteger(1, data, 96, 32);
                var yBC = new Org.BouncyCastle.Math.BigInteger(1, data, 128, 32);
                var point = curveParams.Curve.CreatePoint(xBC, yBC);
                if (!point.IsValid())
                    return new byte[0];

                var rBC = new Org.BouncyCastle.Math.BigInteger(1, data, 32, 32);
                var sBC = new Org.BouncyCastle.Math.BigInteger(1, data, 64, 32);
                var pubKey = new Org.BouncyCastle.Crypto.Parameters.ECPublicKeyParameters(point, domainParams);
                var signer = new Org.BouncyCastle.Crypto.Signers.ECDsaSigner();
                signer.Init(false, pubKey);

                if (signer.VerifySignature(hash, rBC, sBC))
                {
