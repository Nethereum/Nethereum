...
namespace Nethereum.Model.Codecs
{
    ...
    /// downstream execution would silently accept it. Geth's
    /// equivalent guard lives in <c>core/types/transaction.go
    /// Transaction.Type</c> + <c>core/state_processor.go</c> rejecting
    /// the block when <c>tx.Type</c> exceeds the chain's active set.</para>
    /// </summary>
