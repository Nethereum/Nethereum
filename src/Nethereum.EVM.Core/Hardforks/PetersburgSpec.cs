...
namespace Nethereum.EVM.Hardforks
{
    ...
    ///   <item>Geth: <c>params/config.go PetersburgBlock</c>,
    ///   <c>core/vm/jump_table.go newConstantinopleInstructionSet</c>
    ///   (opcode table unchanged from Constantinople),
    ///   <c>core/vm/gas_table.go gasSStore</c> (legacy SSTORE path
    ///   re-selected when <c>!IsEIP1283F</c>).</item>
    /// </list>
    /// </summary>
