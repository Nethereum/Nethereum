...
namespace Nethereum.EIP6963WalletInterop
{
    ...
    {
        private readonly IEIP6963WalletInterop _walletInterop;
        ...
        public EIP6963WalletInterceptor(IEIP6963WalletInterop walletInterop, bool useOnlySigningWalletTransactionMethods = false)
        {
            ...
            string route = null)
        {
            ...
            if (_useOnlySigningWalletTransactionMethods == false || (_useOnlySigningWalletTransactionMethods && SigningWalletTransactionsMethods.Contains(request.Method)))
            {
                ...
                if (request.Method == ApiMethods.eth_sendTransaction.ToString())
                {
                    ...
                    var response = await _walletInterop.SendAsync(new EIP6963RpcRequestMessage(request.Id, request.Method, SelectedAccount,
                ...
                else if (request.Method == ApiMethods.eth_estimateGas.ToString() || request.Method == ApiMethods.eth_call.ToString())
                {
                    ...
                    var response = await _walletInterop.SendAsync(new RpcRequestMessage(request.Id,
                        request.Method,
                        request.RawParameters)).ConfigureAwait(false);
                    return ConvertResponse<T>(response);
                }
                ...
                else if (request.Method == ApiMethods.wallet_watchAsset.ToString())
                {
                    ...
                    var requestMessage = new EIP6963RpcRequestMessage(request.Id, request.Method, SelectedAccount,
                       parameters);
                    requestMessage.RawParameters = parameters; // Ensure RawParameters is a just an object
                    var response = await _walletInterop.SendAsync(requestMessage).ConfigureAwait(false);
                    return ConvertResponse<T>(response);
                }    
                else
                {
            ...
            string route = null, params object[] paramList)
        {
            if (_useOnlySigningWalletTransactionMethods == false || (_useOnlySigningWalletTransactionMethods && SigningWalletTransactionsMethods.Contains(method)))
            {
                ...
                if (method == ApiMethods.eth_sendTransaction.ToString())
                {
                    ...
                    var response = await _walletInterop.SendAsync(new EIP6963RpcRequestMessage(route, method, SelectedAccount,
                ...
                else if (method == ApiMethods.eth_estimateGas.ToString() || method == ApiMethods.eth_call.ToString())
                {
                    ...
                    var response = await _walletInterop.SendAsync(new RpcRequestMessage(route, method,
                         paramList)).ConfigureAwait(false);
                    return ConvertResponse<T>(response);
                }
                ...
                else if (method == ApiMethods.wallet_watchAsset.ToString())
                {
                    ...
                    var requestMessage = new EIP6963RpcRequestMessage(route, method, SelectedAccount,
                      parameters);
                    requestMessage.RawParameters = parameters; // Ensure RawParameters is a just an object
                    var response = await _walletInterop.SendAsync(requestMessage).ConfigureAwait(false);
                    return ConvertResponse<T>(response);
