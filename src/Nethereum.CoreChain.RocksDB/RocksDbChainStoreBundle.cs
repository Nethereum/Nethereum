    ...
    public sealed class RocksDbChainStoreBundle : IChainStoreBundle
    {
        ...
        public static RocksDbChainStoreBundle Open(string dataDir, HistoricalStateOptions journalOptions = null)
        {
            ...
            return new RocksDbChainStoreBundle(
                ...
                new RocksDbWithdrawalStore(rocks, blocks),
        ...
        {
            // Surgical: clear ONLY the snap-sync-state metadata key (not the
            // broader cursor / genesis flag) and wipe ONLY the state-related
            // column families. Receipts, logs, blooms, log indexes, filters
            // and message-result indexes are validated archive data from the
            // Phase 1 backfill and stay untouched. The metadata cursors
            // (LastFetchedHeader / LastFetchedBody / LastBlock) also stay,
            // so the parallel backfiller resumes exactly where it left off.
