...
namespace Nethereum.CoreChain.RocksDB.Stores
{
    ...
    public sealed class RocksDbChainMetadataStore : IChainMetadataStore
    {
        ...
        public SnapSyncState GetSnapSyncState()
        {
            ...
            // Corrupt row → null so the bootstrapper enters the fresh-snap path
            // rather than crashing the host on a bad metadata read.
        ...
        public void SaveSnapSyncState(SnapSyncState state)
        {
            ...
            _rocks.Write(batch);
            // No Flush — WAL durability is enough for snap resume; per-chunk
            // fsync would dominate sync throughput.
        }
