...
namespace Uport
{
   public class UportRegistryService
   {
        ...
        public Task<byte[]> GetAsyncCall(byte[] registrationIdentifier, string issuer, string subject)
        {
           var function = GetFunctionGet();
           return function.CallAsync<byte[]>(registrationIdentifier, issuer, subject);
        }
