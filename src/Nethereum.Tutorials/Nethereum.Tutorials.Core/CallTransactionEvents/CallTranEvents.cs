...
namespace Nethereum.Tutorials
{
    public class CallTranEvents
    {
        ...
        public async Task ShouldBeAbleCallAndReadEventLogs()
        {
          ...
          var filter = await multiplyEvent.CreateFilterAsync();
         ...
         var log = await multiplyEvent.GetFilterChanges<MultipliedEvent>(filter);
         var log7 = await multiplyEvent.GetFilterChanges<MultipliedEvent>(filter7);

         Assert.Equal(2, log.Count);
         Assert.Equal(1, log7.Count);
         Assert.Equal(7, log7[0].Event.MultiplicationParameter);
        ...
        public class MultipliedEvent
        {
            ...
            public int MultiplicationParameter {get; set;}
