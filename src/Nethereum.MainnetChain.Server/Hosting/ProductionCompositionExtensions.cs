...
namespace Nethereum.MainnetChain.Server.Hosting
{
    public static class ProductionCompositionExtensions
    {
            ...
            MainnetChainServerConfig config)
        {
            ...
            services.AddSingleton<IPeerPool>(sp =>
                sp.GetRequiredService<ProductionRuntime>().Pool);

            services.AddSingleton<IFetchRequestScheduler>(sp =>
                sp.GetRequiredService<ProductionRuntime>().Scheduler);

            services.AddSingleton<BackwardBlockWalker>(sp =>
            {
                var runtime = sp.GetRequiredService<ProductionRuntime>();
            ...
            services.AddSingleton<BackwardWalkerDelegate>(sp =>
            {
                ...
                return async (fromBlock, fromHash, toBlock, bundle, ct) =>
                {
                    ...
                    var result = await walker.WalkAsync(fromBlock, fromHash, toBlock, lookup, ct).ConfigureAwait(false);
    ...
    internal sealed class ProductionRuntime : IHostedService, IAsyncDisposable
    {
        ...
        public IPeerPool Pool =>
            _pool ?? throw new InvalidOperationException("ProductionRuntime not initialised.");

        public IFetchRequestScheduler Scheduler =>
            _scheduler ?? throw new InvalidOperationException("ProductionRuntime not initialised.");
