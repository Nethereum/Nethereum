...
namespace Nethereum.DevP2P.Sync.Strategies
{
    ...
    /// Used when our node is a block source for incoming peers (Geth followers,
    /// other Nethereum followers, AppChain replicas).
    /// <para>
    /// Missing-data semantics follow geth's <c>eth/protocols/eth/handlers.go</c>:
    ...
    /// then retries against another server. See
    /// <c>ServiceGetBlockBodiesQuery</c> (handlers.go:232), <c>ServiceGetReceiptsQuery68</c>
    /// (handlers.go:273), and <c>serviceContiguousBlockHeaderQuery</c>
    /// (handlers.go:157).
    /// </para>
    /// </summary>
