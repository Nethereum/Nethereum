...
namespace Nethereum.DevP2P.Sync
{
    ...
    public sealed class PeerPoolManager : IPeerPool
    {
        ...
        // Optional outbound dial scheduler (concurrent-cap +
        ...
        // Outbound subnet diversity gate (eclipse defence):
        // at most MaxPeersPerIPv4Subnet peers per /24 (and
        // MaxPeersPerIPv6Subnet per /64). Without this a /24 owner can fill
        // the entire outbound pool through hundreds of distinct IPs without
        // tripping any per-host gate. Resolved peer IP per enode is cached
        // so disconnect-time release uses the same address that admission
        // used, even if DNS later changes.
        ...
        // entries older than BannedRetention age out: bans
        ...
        private async Task DialLoopAsync(CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                foreach (var candidate in batch)
                {
                    ...
                    // 3a) Subnet diversity (eclipse defence): reject the
                    //     candidate if its /24 IPv4 or /64 IPv6 quota is full.
                    //     Trusted peers bypass. Address resolved here is
                    //     cached so disconnect releases the same slot.
                    var isTrustedCandidate = _trustedDialKeys.Contains(candidate);
                    if (!TryReserveSubnetSlot(candidate, isTrustedCandidate, out var subnetAddress))
                    {
                        _logger.LogDebug(
                            "subnet diversity rejected {Host}", MainnetPeerSession.ParseHost(candidate));
                    ...
                    // 4) Dial scheduler — concurrent-cap +
        ...
        /// <summary>
        /// Reserve a per-subnet slot for the resolved IP of <paramref name="enode"/>.
        /// Trusted peers bypass the subnet cap (same convention as the dial
        /// scheduler — operators that explicitly trust a peer accept the
        /// concentration risk). Returns true (and records the address for
        /// later release) on admission. Returns false when the subnet is
        /// full, in which case the candidate must be released back through
            ...
            string enode, DialCandidate? schedCandidate, CancellationToken ct)
        {
            ...
            try
            {
                ...
                var mp = peer as MainnetPeerSession;
                _logger.LogInformation("peer added: {Host} eth/{EthVersion} latest={LatestBlock} caps=[{Caps}] supportsSnap={SupportsSnap} client={Client}",
                    MainnetPeerSession.ParseHost(enode), peer.EthVersion, peer.PeerLatestBlock,
                    mp?.CapabilitiesDescription ?? "?", mp?.SupportsSnap ?? false, mp?.PeerClientId ?? "?");
            ...
            finally
            {
                ...
                // Failed dials must free the subnet slot; successful dials
                // keep it until OnPeerDisconnected fires.
