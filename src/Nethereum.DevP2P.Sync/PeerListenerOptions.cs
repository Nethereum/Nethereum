...
namespace Nethereum.DevP2P.Sync
{
    ...
    public sealed class PeerListenerOptions
    {
        ...
        /// <summary>
        /// Fires on each successful inbound admission (after the eth Status
        /// exchange completes). Caller may use this to feed an outbound
        /// <see cref="Nethereum.DevP2P.Peering.DialScheduler"/> via
        /// <c>OnPeerConnected(key, PeerDirection.Inbound)</c> so the
        /// inbound/outbound ratio cap is accurate, or for metrics. The first
        /// argument is a stable peer key (remote node id hex when known, else
        /// remote endpoint). Exceptions thrown by the handler are swallowed;
        /// do not let observability errors tear down the listener.
        /// </summary>
        public Action<string> OnInboundPeerAdded { get; set; }

        /// <summary>
        /// Fires when an inbound peer leaves the pool. Pairs with
        /// <see cref="OnInboundPeerAdded"/> 1:1 — the same peer key is passed.
        /// </summary>
