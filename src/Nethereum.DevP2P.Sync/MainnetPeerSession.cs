...
namespace Nethereum.DevP2P.Sync
{
    ...
    public sealed class MainnetPeerSession : IAsyncDisposable, IDisposable, IEthPeer
    {
        ...
        /// bootnodes (verbatim copy of <c>params.MainnetBootnodes</c> from
        /// go-ethereum + Erigon — both publish the same four). The remainder
        ...
        /// Mirrors the geth eth/handlers.go anchor-by-hash path in
            ...
            byte[] stateRoot, byte[] startingHash, byte[] limitHash, ulong responseBytes, CancellationToken ct)
        {
            ...
            await _conn.SendMessageAsync(snapOffset + SnapMessageIds.GetAccountRange,
                GetAccountRangeMessageEncoder.Encode(req), ct);
            return await AwaitSnapResponseAsync<AccountRangeMessage>(
