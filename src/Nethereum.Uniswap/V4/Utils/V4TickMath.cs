...
namespace Nethereum.Uniswap.V4.Utils
{
    public class V4TickMath
    {
        ...
        public decimal GetPriceAtTick(int tick)
        {
            ...
            return PriceCalculator.Current.CalculatePriceFromSqrtPriceX96(sqrtPriceX96);
        }

        public decimal GetPriceAtTick(int tick, int decimals0, int decimals1)
        {
            var sqrtPriceX96 = GetSqrtRatioAtTick(tick);
            return PriceCalculator.Current.CalculatePriceFromSqrtPriceX96(sqrtPriceX96, decimals0, decimals1);
        }

        public int GetTickAtPrice(decimal price)
        {
            var sqrtPriceX96 = PriceCalculator.Current.CalculateSqrtPriceX96FromPrice(price);
            return GetTickAtSqrtRatio(sqrtPriceX96);
        }

        public int GetTickAtPrice(decimal price, int decimals0, int decimals1)
        {
            var sqrtPriceX96 = PriceCalculator.Current.CalculateSqrtPriceX96FromPrice(price, decimals0, decimals1);
            return GetTickAtSqrtRatio(sqrtPriceX96);
        }
