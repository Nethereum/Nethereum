...
/// }    
    ...
    public class EthSign : RpcRequestResponseHandler<string>
        {
            public EthSign() : base(ApiMethods.eth_sign.ToString()) { }

            public async Task<string> SendRequestAsync(RpcClient client, string address, string data, string id = Constants.DEFAULT_REQUEST_ID)
            {
                return await base.SendRequestAsync(client, id, address, data);
            }
