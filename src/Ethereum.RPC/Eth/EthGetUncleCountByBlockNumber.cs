...
/// }    
    ...
    public class EthGetUncleCountByBlockNumber : RpcRequestResponseHandler<HexBigInteger>
        {
            public EthGetUncleCountByBlockNumber() : base(ApiMethods.eth_getUncleCountByBlockNumber.ToString()) { }

            public async Task<HexBigInteger> SendRequestAsync(RpcClient client, HexBigInteger blockNumber, string id = Constants.DEFAULT_REQUEST_ID)
            {
                return await base.SendRequestAsync(client, id, blockNumber);
            }
