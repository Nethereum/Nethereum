...
{
    /// <summary>
    /// Static list of mainnet discv5 bootnode ENRs operated by the Ethereum
    /// Foundation and major client teams. Use as a fallback when DNS-based
    /// discovery (<see cref="Nethereum.DevP2P.Dns.EnrTreeResolver"/>) is
    /// unreachable.
    /// </summary>
    public static class Discv5Bootnodes
    {
        /// <summary>
        /// Mainnet V5 bootnode ENR URLs. Operated by Teku, Prylab, Lighthouse,
        /// Ethereum Foundation, and Nimbus teams.
        /// </summary>
        public static readonly string[] Mainnet = new[]
        {
        ...
        /// <summary>
        /// Decode each <see cref="Mainnet"/> ENR and yield the (record, endpoint)
        /// pair ready to feed into <see cref="Discv5Listener.SendPingAsync"/>.
        /// Records with no IPv4 or UDP port are skipped.
        /// </summary>
        public static IEnumerable<(EnrRecord Enr, IPEndPoint Address)> ResolveMainnet()
        {
            foreach (var url in Mainnet)
            {
                EnrRecord enr;
                try { enr = EnrRecordEncoder.ParseUrl(url); }
                catch (System.Exception) { continue; }
                if (enr.IP4 == null || enr.UdpPort == null) continue;
                yield return (enr, new IPEndPoint(enr.IP4, enr.UdpPort.Value));
            }
        }
    }
}
