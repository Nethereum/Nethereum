...
var enodeUrl = EnrRecordEncoder.ToUrl(enr);   // share this with other nodes
```
...
var range = await snapHandler.GetAccountRangeCoreAsync(stateRoot, startHash, limitHash, byteLimit, ct);
// `range` carries the slim accounts and the edge proof the peer needs to verify.
```
