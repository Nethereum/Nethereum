...
namespace Nethereum.Contracts.IntegrationTests.SmartContracts
{
    ...
    public class NonTypeSafeInteractionTests
    {
        private readonly EthereumClientIntegrationFixture _ethereumClientIntegrationFixture;

        public NonTypeSafeInteractionTests(EthereumClientIntegrationFixture ethereumClientIntegrationFixture)
        {
        ...
        [NethereumDocExample(DocSection.SmartContracts, "smart-contract-interaction",
            "Deploy and interact using ABI JSON (non-type-safe)",
            SkillName = "smart-contract-interaction", Order = 8)]
        public async Task DeployAndInteract_UsingAbiJson()
        {
            ...
            var balance = await balanceFunction.CallAsync<BigInteger>(senderAddress);
            ...
            var transferReceipt = await transferFunction.SendTransactionAndWaitForReceiptAsync(
                senderAddress, gas, null, null, newAddress, amountToSend);

            Assert.NotNull(transferReceipt.TransactionHash);

            var newBalance = await balanceFunction.CallAsync<BigInteger>(newAddress);
            Assert.Equal(amountToSend, newBalance);
        }
    }
}
