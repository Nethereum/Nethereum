...
namespace Nethereum.Contracts.IntegrationTests.EVM
{
    ...
    public class Erc20EVMContractSimulatorAndStorage
    {
        ...
        public async void ShouldCalculateBalanceSlot()
        {
            ...
            var simulator = new Nethereum.EVM.Contracts.ERC20.ERC20Simulator();
            var slot = await simulator.CalculateMappingBalanceSlotAsync(contractAddress, senderAddress, 1, web3, 100);
            Assert.Equal(9, slot);
        }
        ...
        public async void ShouldSimulateTransferAndBalanceState()
        {
            ...
            var simulator = new Nethereum.EVM.Contracts.ERC20.ERC20Simulator();
            var simulationResult = await simulator.SimulateTransferAndBalanceStateAsync(contractAddress, senderAddress, receiverAddress, 100, web3, 1);
