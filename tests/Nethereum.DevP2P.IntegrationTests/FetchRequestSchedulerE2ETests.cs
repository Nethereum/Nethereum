    ...
    public class FetchRequestSchedulerE2ETests
    {
        ...
        private sealed class ControlledRequestWorker : IPeerRequestWorker
        {
            ...
            public Task<List<List<Receipt>>> GetReceiptsAsync(IEthPeer peer, IReadOnlyList<byte[]> blockHashes, CancellationToken ct)
                => throw new NotImplementedException("Receipts not exercised in Stage 3.1 tests");

            public Task<Nethereum.Model.P2P.Snap.AccountRangeMessage> GetAccountRangeAsync(IEthPeer peer, byte[] stateRoot, byte[] startingHash, byte[] limitHash, ulong responseBytes, CancellationToken ct)
                => throw new NotImplementedException("snap not exercised in Stage 3.1 tests");

            public Task<Nethereum.Model.P2P.Snap.StorageRangesMessage> GetStorageRangesAsync(IEthPeer peer, byte[] stateRoot, List<byte[]> accountHashes, byte[] startingHash, byte[] limitHash, ulong responseBytes, CancellationToken ct)
                => throw new NotImplementedException("snap not exercised in Stage 3.1 tests");

            public Task<Nethereum.Model.P2P.Snap.ByteCodesMessage> GetByteCodesAsync(IEthPeer peer, List<byte[]> codeHashes, ulong responseBytes, CancellationToken ct)
                => throw new NotImplementedException("snap not exercised in Stage 3.1 tests");

            public Task<Nethereum.Model.P2P.Snap.TrieNodesMessage> GetTrieNodesAsync(IEthPeer peer, byte[] stateRoot, List<List<byte[]>> paths, ulong responseBytes, CancellationToken ct)
                => throw new NotImplementedException("snap not exercised in Stage 3.1 tests");
