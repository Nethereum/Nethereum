...
namespace Nethereum.DevP2P.IntegrationTests
{
    public class DevP2PBlockSourceE2ETests
    {
        ...
        private sealed class StubScheduler : IFetchRequestScheduler
        {
            ...
            public Task<BodyFetchResult> FetchBodiesAsync(IReadOnlyList<byte[]> blockHashes, IReadOnlyCollection<System.Guid>? excludePeers, CancellationToken ct)
                => throw new System.NotSupportedException("not exercised in this test");

            public Task<List<List<Receipt>>> FetchReceiptsAsync(IReadOnlyList<byte[]> blockHashes, CancellationToken ct)
                => throw new System.NotSupportedException("not exercised in this test");

            public Task<Nethereum.Model.P2P.Snap.AccountRangeMessage> FetchAccountRangeAsync(byte[] stateRoot, byte[] startingHash, byte[] limitHash, ulong responseBytes, CancellationToken ct)
                => throw new System.NotSupportedException("not exercised in this test");

            public Task<Nethereum.Model.P2P.Snap.StorageRangesMessage> FetchStorageRangesAsync(byte[] stateRoot, List<byte[]> accountHashes, byte[] startingHash, byte[] limitHash, ulong responseBytes, CancellationToken ct)
                => throw new System.NotSupportedException("not exercised in this test");

            public Task<Nethereum.Model.P2P.Snap.ByteCodesMessage> FetchByteCodesAsync(List<byte[]> codeHashes, ulong responseBytes, CancellationToken ct)
                => throw new System.NotSupportedException("not exercised in this test");

            public Task<Nethereum.Model.P2P.Snap.TrieNodesMessage> FetchTrieNodesAsync(byte[] stateRoot, List<List<byte[]>> paths, ulong responseBytes, CancellationToken ct)
                => throw new System.NotSupportedException("not exercised in this test");
        }
