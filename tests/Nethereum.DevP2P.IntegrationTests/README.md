...
```bash
cd C:/Users/SuperDev/Documents/Repos
...
cd C:\Users\SuperDev\Documents\Repos\go-ethereum
go build -o C:\Users\SuperDev\Documents\Repos\Nethereum\geth-tools\devp2p.exe .\cmd\devp2p\
```
...
For the eth-test mempool sub-tests (`InvalidTxs`, `LargeTxRequest`, blob variants), the historical state at head is rebuilt by `GethTestdataHistoricalStateBuilder` replaying every block through `Nethereum.CoreChain.BlockProcessor`. The result is cached statically so the ~2 s cost is paid once per process.
