...
namespace Nethereum.Signer.UnitTests
{
    public class TransactionTests
    {
        ...
        public void ShouldGenerateECKey()
        {
            ...
            Assert.Equal(ecKey.GetPublicAddress().EnsureHexPrefix().ToLower(),
                regeneratedKey.GetPublicAddress().EnsureHexPrefix().ToLower());
        }
        ...
        public void ShouldSignEncodeTransactionAndRecoverPublicAddress()
        {
            ...
            Assert.Equal(sendersAddress.EnsureHexPrefix().ToLower(),
                key.GetPublicAddress().EnsureHexPrefix().ToLower());
        }
        ...
        {
            var tx = new LegacyTransaction(RLP_ENCODED_UNSIGNED_TX.HexToByteArray());
            Assert.Null(tx.Signature);
        }
    }
}