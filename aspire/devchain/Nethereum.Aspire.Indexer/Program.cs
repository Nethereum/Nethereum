...
{
    // EnsureCreatedAsync skips when database already has tables from other contexts.
    // Generate idempotent SQL and execute each statement independently so shared
    // tables/constraints (BlockProgress, ChainStates) that already exist don't block
    // creation of MUD-specific tables (StoredRecords).
    ...
    foreach (var statement in script.Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
    {
        ...
        catch (PostgresException ex) when (ex.SqlState == "42P07") // 42P07 = duplicate_table
        {
        }
    }
}
