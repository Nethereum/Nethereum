...
namespace Nethereum.Gen
{
    class MainClass
    {
        ...
        {
            string abi = @"[{""constant"":false,""inputs"":[{""name"":""symbol"",""type"":""bytes32""}],""name"":""getToken"",""outputs"":[{""name"":"""",""type"":""address""}],""type"":""function""}]";
            var temp = new Templates();
            Debug.Write(temp.ContractGen(abi, "Registry"));
        }
    }
...
{{
    ...
    return await function.SendTransactionAsync(addressFrom, gas, valueAmount, {1});
        ...
        public string ContractGen(string abi, string contractName)
        {
            ...
            return string.Format(ContractTemplate, abi.Replace("\"", "\"\""), contractName, operations);
        }
