...
namespace Nethereum.BlockReplay
{
    ...
    internal static class Program
    {
        ...
        private static void PrintHelp()
        {
            ...
            Console.WriteLine("  --emit-fixture <path> write a MainnetBlockFixture JSON file (for Debug_BlockReplay_DiffWithGethT8n)");
            Console.WriteLine("  --trace-tx <i>        enable opcode tracing for tx index i (production path; pairs with --out-trace)");
            Console.WriteLine("  --out-trace <path>    geth-compatible JSONL trace dump of the --trace-tx tx");
        ...
        // Geth-compatible JSONL: one step per line with {pc, op, gas, gasCost, depth, stack}.
        // Memory/storage omitted to keep diff focused on control flow + value flow.
        // Depth in ProgramTrace is 0-indexed (0 = top-level frame); geth's debug_traceTransaction
        // uses 1-indexed depth (1 = top-level call). We bump by 1 here for direct comparability.
